...
```toml
[dependencies]
rust_ocr_transformer = { git = "https://github.com/arabangoo/rust_ocr_transformer", tag = "v0.1.1" }
```
...
- **읽기순서 정렬 (XY-Cut).** 연결요소 추출은 래스터 스캔 순서라 단어가 뒤섞여 나온다. 검출기는
  ...
  방해하기 때문이다. `ROCT_XYCUT_GAP` 또는 `with_xycut_gap` 으로 분할 간격을 조정하고, 단순 줄 묶기
...
- **적응형 검출 해상도.** 검출 입력을 입력 이미지 크기에 비례해 정한다(CLI `--det-long`, 긴 변 목표
  ...
  고정값이 필요하면 `ROCT_DET_H`/`ROCT_DET_W` 로 덮어쓴다. 인식 입력은 48x320 고정(매우 넓은 줄은
  잘릴 수 있음).
...
- **`--auto-rotate`** — 0/90/180/270° 중 OCR 신뢰도(고신뢰 영역 수)가 가장 높은 방향을 고른다.
  ...
  0° 채택 기준 점수는 `ROCT_ORIENT_OK`(기본 8)로 조정한다.
- **`--det-long N`** — 검출 입력의 긴 변 목표 px(32 배수로 반올림). 작은 글자가 안 잡히면 올린다
  (느려진다). `ROCT_DET_H`/`ROCT_DET_W` 로 형상을 직접 고정할 수도 있다.
- **`ROCT_UNCLIP`** — DB unclip 비율(기본 1.5). 글자 가장자리가 잘리면 키운다.
